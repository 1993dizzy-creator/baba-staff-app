...
export default function PartTimeExtraWorkSection({ employee, month, lang, refresh }: {
  ...
  return <section style={s.section} aria-label={vi ? "Làm thêm part-time" : "파트타임 추가근무"}>
    ...
    {rows.length === 0 ? <small style={s.help}>{vi ? "Không có thời gian làm thêm đủ điều kiện trong tháng này." : "이번 달 지급 후보 추가근무가 없습니다."}</small> : <>
      ...
      {isExpanded && <div id={listId} style={s.list}>{rows.map(row => {
        const disabled = paid || busyId === row.attendanceRecordId;
        return <article key={row.attendanceRecordId} style={s.item}>
      ...
      <div style={s.actions}>
        {(row.status === "review_required" || row.status === "stale") ? <><button type="button" disabled={disabled} style={{ ...s.approve, ...(disabled ? s.disabledButton : s.enabledButton) }} onClick={() => decide(row.attendanceRecordId, "approved", row.sourceHash)}>{vi ? "Duyệt" : "승인"}</button><button type="button" disabled={disabled} style={{ ...s.reject, ...(disabled ? s.disabledButton : s.enabledButton) }} onClick={() => decide(row.attendanceRecordId, "rejected", row.sourceHash)}>{vi ? "Từ chối" : "거절"}</button></> : <button type="button" disabled={disabled} style={{ ...s.cancel, ...(disabled ? s.disabledButton : s.enabledButton) }} onClick={() => cancel(row.attendanceRecordId)}>{vi ? "Hủy quyết định" : "결정 취소"}</button>}
      </div>
    </article>})}</div>}
    </>}
...
const s = {
  ...
  help: { color: "#64748b", lineHeight: 1.4 }, toggle: { justifySelf: "start", padding: "3px 7px", border: "1px solid #cbd5e1", borderRadius: 5, background: "#fff", color: "#475569", fontSize: 10, fontWeight: 700, cursor: "pointer" }, list: { display: "grid", gap: 3, maxHeight: 360, overflowY: "auto", paddingRight: 2 }, item: { display: "grid", gridTemplateColumns: "36px minmax(0,1fr) auto auto", gap: 5, alignItems: "center", minHeight: 28, padding: "4px 6px", border: "1px solid #e2e8f0", borderRadius: 7, background: "#fff" },
  ...
  enabledButton: { cursor: "pointer" }, disabledButton: { cursor: "not-allowed", opacity: 0.55 },
