...
export default function BarLogEntry({ log, lang, compact = false }: { log: BarActivityLog; lang: "ko" | "vi"; compact?: boolean }) {
  ...
  const isDeletedKeeping = log.entityType === "keeping" && log.actionType === "keeping_deleted";
  const target = log.entityCode || (log.entityType === "zone" ? `#${log.entityId}` : `#${log.entityId}`);
  const targetStyle: React.CSSProperties = { color: "#1f2937", fontSize: compact ? 12 : 13, fontWeight: 800, lineHeight: 1.4, overflowWrap: "anywhere", textDecoration: log.entityType === "keeping" && !isDeletedKeeping ? "underline" : "none", textDecorationColor: "#9ca3af", textUnderlineOffset: 2 };
  ...
  return <article style={compact ? { padding: "9px 0", borderTop: "1px solid #f3f4f6" } : { ...ui.card, padding: 12 }}>
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", alignItems: "start", gap: 8 }}>
      {log.entityType === "keeping" && !isDeletedKeeping ? <Link href={`/bar/keeping/${log.entityId}`} style={{ ...targetStyle, minWidth: 0 }}>{target}</Link> : <div style={{ ...targetStyle, minWidth: 0 }}>{target}</div>}
