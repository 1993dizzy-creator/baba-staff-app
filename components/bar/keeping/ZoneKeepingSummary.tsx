...
type Item = { id: number; customerName: string; liquorName: string; remainingPercent: number; thumbnailUrl: string | null };
export default function ZoneKeepingSummary({ zoneCode, lang, refreshKey }: { zoneCode: string; lang: "ko" | "vi"; refreshKey: number }) {
  ...
  return <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid #e5e7eb" }}>
    ...
    {loading ? <p style={muted}>{t.loading}</p> : error ? <p role="alert" style={{ ...muted, color: "#b91c1c" }}>{error}</p> : items.length === 0 ? <p style={muted}>{t.noActive}</p> : <div style={{ marginTop: 7 }}>
      {items.map((item) => <Link key={item.id} href={`/bar/keeping/${item.id}?from=${encodeURIComponent(`/bar/keeping?status=active&zone=${zoneCode}`)}`} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 0", borderTop: "1px solid #f3f4f6", color: "inherit", textDecoration: "none" }}><div style={{ width: 44, height: 52, flex: "0 0 auto", borderRadius: 8, overflow: "hidden", background: "#f3f4f6" }}>{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : null}</div><div style={{ minWidth: 0, flex: 1 }}><div style={ellipsis}>{item.customerName}</div><div style={{ ...ellipsis, fontSize: 11, color: "#6b7280", fontWeight: 400 }}>{item.liquorName}</div></div><strong style={{ flexShrink: 0, fontSize: 12 }}>{item.remainingPercent}%</strong></Link>)}
      {total > items.length ? <Link href={`/bar/keeping?status=active&zone=${zoneCode}`} style={link}>+{total - items.length} {lang === "vi" ? "chai nữa" : "병 더 보기"}</Link> : null}
    </div>}
  </div>;
}
