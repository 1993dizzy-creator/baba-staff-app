  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    <section style={{ border: "1px solid #dcdfe4", borderRadius: 18, background: "#fff", overflow: "hidden", boxShadow: "0 6px 20px rgba(0,0,0,.04)" }}>
      ...
      <div style={{ padding: 15 }}>
        ...
        {item.status === "closed" && (capabilities.reactivate || capabilities.editClosed) ? <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>{capabilities.reactivate ? <button ref={actionRef} onClick={() => setAction("reactivate")} style={primaryButtonStyle}>{t.reactivate}</button> : <span />}{capabilities.editClosed ? <button onClick={() => setAction("update")} style={secondaryButtonStyle}>{t.edit}</button> : null}</div> : null}
