  ...
  return <div style={{ padding: "0 0 20px" }}>
    ...
    <section style={{ border: "1px solid #dcdfe4", borderRadius: 18, background: "#fff", overflow: "hidden", boxShadow: "0 6px 20px rgba(0,0,0,.04)" }}>
      ...
      <div style={{ padding: 15 }}>
        ...
        {item.status === "closed" && (capabilities.reactivate || capabilities.delete) ? <div style={{ display: "grid", gridTemplateColumns: capabilities.reactivate && capabilities.delete ? "minmax(0,1fr) minmax(0,1fr)" : "1fr", gap: 8, marginTop: 12 }}>{capabilities.reactivate ? <button ref={actionRef} onClick={() => setAction("reactivate")} style={primaryButtonStyle}>{t.reactivate}</button> : null}{capabilities.delete ? <button ref={deleteRef} type="button" onClick={() => setDeleteOpen(true)} style={{ ...secondaryButtonStyle, borderColor: "#fca5a5", color: "#b91c1c", background: "#fff7f7" }}>{lang === "vi" ? "Xóa" : "삭제"}</button> : null}</div> : null}
