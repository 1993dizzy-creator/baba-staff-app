...
export default function KeepingActionModal({ item, action, lang, onClose, onSaved, returnFocusRef }: Props) {
  ...
  const [image, setImage] = useState<KeepingImageFiles | null>(null), [saving, setSaving] = useState(false), [error, setError] = useState("");
  ...
  const selectUpdateZone = (code: string) => { set("zoneCode", code); if (code === item.zoneCode) set("reason", ""); setZoneExpanded(false); };
  ...
  async function save(event: React.FormEvent) {
    ...
    if (action === "update" && zoneChanged && !String(values.reason).trim()) { setError(t.invalid); return; }
    setSaving(true); setError("");
    try {
      const payload = { ...values, ...(action === "update" ? { liquorSource: source, inventoryItemId: product?.id ?? null, zoneChanged } : null) };
  ...
  return <form onSubmit={save}><BarSheet kind={kind} compact={action === "use" || action === "replace_photo"} title={title} closeLabel={t.cancel} saving={saving} onClose={onClose} returnFocusRef={returnFocusRef} footer={<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><button type="button" disabled={saving} onClick={onClose} style={secondaryButtonStyle}>{t.cancel}</button><button disabled={saving} style={effectiveAction === "close" ? dangerButtonStyle : primaryButtonStyle}>{saving ? t.saving : t.save}</button></div>}>
