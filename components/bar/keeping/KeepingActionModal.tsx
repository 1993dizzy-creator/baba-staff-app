...
export default function KeepingActionModal({ item, action, lang, onClose, onSaved, returnFocusRef }: Props) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const payload: Record<string, unknown> = { ...values, ...(["use", "correct_remaining", "close"].includes(effectiveAction) ? { note: String(values.actionNote) } : null), ...(action === "update" ? { liquorSource: source, inventoryItemId: product?.id ?? null, zoneChanged } : null) };
      if (submitAction === "move" || submitAction === "update_with_move") delete payload.reason;
