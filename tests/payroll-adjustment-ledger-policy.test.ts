...
test("current amount includes adjustments exactly once while advance remains outside pre-insurance pay",()=>{assert.match(overview,/incentiveAmount=manualIncentiveAmount\+automaticIncentiveAmount/);assert.match(overview,/calculatePayrollPayoutAmounts/);assert.match(overview,/employeeInsuranceDeductionAmount,advanceAmount/);assert.match(overview,/automaticPenaltyAmount\+manualPenaltyAmount/)});
