...
test('normalizes facts in integer minutes without creating the obsolete overtime-unavailable warning',()=>{const facts=normalizeAttendanceDayFacts({userId:7,businessDate:'2026-08-03',schedule,attendanceRecord:{id:4,status:'done',checkInAt:'2026-08-03T08:50:00.000Z',checkOutAt:'2026-08-03T18:10:00.000Z',approvalStatus:'approved'}});assert.equal(facts.scheduledMinutes,540);assert.equal(facts.actualMinutes,560);assert.equal(facts.scheduledOverlapMinutes,540);assert.equal(facts.overtimeCandidateMinutes,20);assert.ok(!facts.warningCodes.includes('OVERTIME_APPROVAL_UNAVAILABLE'));assert.equal(facts.payrollStatus,'calculable');});
...
test('minute and hour projections consume the exact same facts object',()=>{const facts=normalizeAttendanceDayFacts({userId:7,businessDate:'2026-08-03',schedule,attendanceRecord:{id:4,status:'done',checkInAt:'2026-08-03T09:00:00.000Z',checkOutAt:'2026-08-03T18:00:00.000Z',approvalStatus:'approved'}});const results=(['minute','hour'] as const).map(b=>projectPayrollAttendanceDay(facts,contract,b));assert.equal(results[0].recognizedMinutes,540);assert.equal(results[1].recognizedMinutes,540);assert.equal(facts.source.engineVersion,'attendance-facts-v2');});
