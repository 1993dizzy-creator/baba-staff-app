...
const base: PayrollContract={id:1,userId:1,payType:"monthly",calculationBasis:"minute",baseSalary:9_500_000,fixedRaiseAmount:500_000,standardWorkdays:25,standardMinutesPerDay:480,timeBlockMinutes:60,roundingMode:"none",lateAdjustmentMode:"separate",earlyLeaveAdjustmentMode:"deduct_minutes",overtimeMode:"ignore",paidLeaveMode:"manual_review",effectiveFrom:"2026-07-01",effectiveTo:null,revision:1};
...
test("minute basis prorates recognized work without a legacy day override",()=>{const result=calculate(base,420,0,30);assert.equal(result.recognizedMinutes,420);assert.equal(result.recognizedWorkdays,420/480);assert.equal(result.workAmount,(380_000/480)*420);assert.equal(result.automaticEarlyLeavePenalty,0);assert.equal(result.deductionEarlyLeaveMinutes,0);});
test("actual-time basis never creates a second early-leave deduction",()=>{for(const basis of ["minute","hour"] as const){const result=calculate({...base,calculationBasis:basis},370,0,110);assert.equal(result.deductionEarlyLeaveMinutes,0);assert.equal(result.automaticEarlyLeavePenalty,0);}});
