...
test("workday basis deducts the complete early-leave minutes without threshold or block rounding",()=>{for(const minutes of [90,91,110]){const result=calculate(base,480-minutes,0,minutes);assert.equal(result.deductionEarlyLeaveMinutes,minutes);assert.equal(result.automaticEarlyLeavePenalty,(380_000/480)*minutes);}assert.notEqual(calculate(base,370,0,110).deductionEarlyLeaveMinutes,90);assert.notEqual(calculate(base,370,0,110).deductionEarlyLeaveMinutes,20);});
