...
test("v9 keeps the full scheduled day and independent v8 late/early-leave penalty policy",()=>{
  ...
  assert.match(engine,/latePenaltyMode:"settings_tier"/);
  assert.match(engine,/latePenaltyMode:"normalized_30min"/);
  assert.match(v7Migration,/monthly-payroll-v7/);
});
test("new contract UI removes the legacy day basis and direct hours",()=>{assert.doesNotMatch(contractUi,/options=\{\s*form\.payType === "hourly"/);assert.doesNotMatch(contractUi,/value=\{form\.standardHoursPerDay\}/);assert.match(contractUi,/현재 근무시간/);assert.match(contractUi,/근무시간 이력/);assert.match(contractUi,/calculationBasis: selectedRequiresFixedMonthly \? "fixed_monthly" : "minute"/);assert.match(labels,/실근무 정산/);assert.doesNotMatch(labels,/근무일 정산/);});
test("common settings keeps one save for ten fields and compact bilingual sections",()=>{for(const field of ["lateMajorThresholdMinutes","lateMinorPenaltyMinutes","lateMajorPenaltyRateBp","unauthorizedAbsencePenaltyDays"])assert.match(settings,new RegExp(field));for(const heading of ["💰","🛡️","⚠️","Lương","Bảo hiểm","Phạt"])assert.match(settings,new RegExp(heading));for(const label of ["Mốc phân loại","Đi muộn 1~","Đi muộn quá","Nghỉ không phép"])assert.match(settings,new RegExp(label));assert.equal((settings.match(/method: "PUT"/g)??[]).length,1);assert.match(settings,/width: 56/);assert.match(settings,/height: 34/);assert.match(settings,/textAlign: "right"/);});
