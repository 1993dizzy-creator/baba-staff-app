...
function getKeepingActionNote(log:BarActivityLog){
...
export function formatBarLogSummary(log:BarActivityLog,lang:BarLanguage,options:{includeTarget?:boolean}={}){
  ...
  const actionNote=getKeepingActionNote(log);const withActionNote=(summary:string)=>actionNote?`${summary} · ${lang==="vi"?"Ghi chú":"비고"}: ${actionNote}`:summary;
  if(log.actionType==="keeping_remaining_corrected"&&beforePercent!==null&&afterPercent!==null)return withActionNote(lang==="vi"?`Đã chỉnh lượng còn lại ${beforePercent}% → ${afterPercent}%${suffix}.`:`${prefix}잔량을 ${beforePercent}% → ${afterPercent}%로 정정했습니다.`);
  ...
  if(log.actionType==="keeping_used"&&afterPercent!==null){const finished=closeReason==="finished";return withActionNote(lang==="vi"?`Đã xử lý sử dụng, còn ${afterPercent}%${finished?" và kết thúc do đã dùng hết":""}${suffix}.`:`${prefix}사용 처리했습니다. 잔량 ${afterPercent}%${finished?", 소진 종료":""}.`);}
  ...
  return withActionNote((lang==="vi"?vi:ko)[log.actionType]??(lang==="vi"?`Đã thay đổi thông tin${suffix}.`:`${prefix}정보를 변경했습니다.`));
}
