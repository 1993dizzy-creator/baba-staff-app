...
export const KEEPING_SELECT = "id,customer_name,customer_identifier,liquor_name,note,zone_code,status,close_reason,close_note,remaining_percent,image_path,thumbnail_path,image_updated_at,stored_at,last_used_at,expires_at,closed_at,version,created_at,updated_at,bar_zones!inner(code,is_active)";
...
export async function mapKeeping(row: Record<string, any>, includeDetail: boolean): Promise<BarKeeping> {
  ...
  return { id:Number(row.id), customerName:row.customer_name, customerIdentifier:row.customer_identifier, liquorName:row.liquor_name, note:row.note,
