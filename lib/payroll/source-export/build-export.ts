...
import type { PayrollSourceAdjustment, PayrollSourceAttendanceItem, PayrollSourceContract, PayrollSourceEmployee, PayrollSourceExport, PayrollSourceExtraWorkItem, PayrollSourceHolidayWorkPremiumItem } from "./types.ts";
...
function holidayWorkPremium(raw: PayrollRunEmployeeInput): PayrollSourceHolidayWorkPremiumItem[] {
  return raw.items.filter(item=>item.category==="holiday_work_premium"&&item.direction==="addition").map(item=>({businessDate:item.businessDate??String(item.sourceSnapshot.holidayDate??""),holidayId:Number(item.sourceSnapshot.holidayId),holidayCode:String(item.sourceSnapshot.holidayCode??""),holidayGroup:String(item.sourceSnapshot.holidayGroup??""),internalPayMultiplier:item.sourceSnapshot.internalPayMultiplier==null?null:Number(item.sourceSnapshot.internalPayMultiplier),effectiveMultiplier:Number(item.sourceSnapshot.effectiveMultiplier),premiumCalculationBaseAmount:Number(item.sourceSnapshot.premiumCalculationBaseAmount),premiumAmount:item.amount}));
}
