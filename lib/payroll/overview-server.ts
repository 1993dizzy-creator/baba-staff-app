...
import { loadPayrollTaxVersions } from "@/lib/payroll/tax-server";
...
export async function loadPayrollOverview(month: string,options?:{userId?:number;onSnapshotReady?:(input:{snapshot:PayrollMonthSnapshot;period:PayrollOverviewPeriod})=>void}) {
  ...
  const [snapshot,adjustmentResult,attendanceStanding,bonusVersions,taxVersions]=await Promise.all([
    snapshotPromise,
    adjustmentPromise,
