...
export function buildEmployeePaymentSnapshot(employee:PayrollOverviewEmployee,raw:PayrollRunEmployeeInput,sourceSnapshot:Record<string,unknown>){
  return {employee,contractSnapshot:raw.contractSnapshot,attendanceSnapshot:raw.attendanceSnapshot,insuranceSnapshot:raw.insuranceSnapshot,taxSnapshot:employee.tax,levelSnapshot:employee.levelInfo,adjustmentsSnapshot:employee.adjustments,automaticIncentivesSnapshot:employee.automaticIncentives??[],automaticPenaltiesSnapshot:employee.automaticPenalties,sourceSnapshot};
}
