...
import type { EmployeeLevelProgramVersion } from "@/lib/employee-level/types";
import { calculateCombinedSalary } from "./compensation";
import { applyUnifiedPayrollWorkPolicy as applyPayrollWorkPolicy, calculatePayrollRates, selectUnifiedRecognizedMinutes } from "./work-policy";
...
export type AttendanceRow={id:number;user_id:number;status:string;work_date:string;check_in_at:string|null;check_out_at:string|null;late_minutes:number|null;early_leave_minutes:number|null;work_minutes:number|null;note?:string|null;approval_status:string|null;updated_at:string|null};
type AutoCategory="base_work"|"paid_leave"|"overtime"|"part_time_extra_work"|"late_deduction"|"early_leave_deduction"|"unauthorized_absence_deduction"|"insurance_employee_deduction";
...
function calculateEmployee(user:PayrollSnapshotUserRow,records:AttendanceRow[],input:BatchInput):PayrollRunEmployeeInput{
  ...
  if(calculationDate&&fixedContractMatches.length===1){
    ...
    return{userId:user.id,employeeName:employeeName(user),contractSnapshot:contracts,attendanceSnapshot:{month:input.month,engineVersion:PAYROLL_RUN_ENGINE_VERSION,calculationBasis:"fixed_monthly",calculationDate,levelSnapshot:fixed?.levelSnapshot??null,days:[]},insuranceSnapshot,recognizedWorkdays:0,recognizedMinutes:0,lateMinutes:0,earlyLeaveMinutes:0,overtimeCandidateMinutes:0,partTimeExtraWork:[],items,reviews};
  }
  for(const date of eligibleDates){const record=recordsByDate.get(date)??null;const contractMatches=activeOn(contracts,date);const scheduleMatches=activeOn(schedules,date);const contract=contractMatches.length===1?contractMatches[0]:null;const schedule=scheduleMatches.length===1?scheduleMatches[0]:null;if(!record&&!schedule)continue;const settings=input.settingsByDate.get(date)??{revision:null,lateGraceMinutes:0,earlyLeaveGraceMinutes:0};
    ...
    if(record?.status==="leave"){
      if(record.approval_status!=="approved"){days.push({date,facts,sourceFact,contractRevision:contract.revision,scheduleRevision:schedule.revision});continue;}
      // No per-record paid-leave marker exists yet, so approved leave remains unpaid by default.
      days.push({date,facts,sourceFact,contractRevision:contract.revision,scheduleRevision:schedule.revision});continue;
    }
    ...
    // 정상근무일: 그날 스케줄 근무시간 전액을 기본급으로 인정한다(지각/조퇴로 분단위 감소 없음).
    const workPolicy=applyPayrollWorkPolicy({contract:dailyContract,actualRecognizedMinutes,dayRate:rate.dayRate,minuteRate:rate.minuteRate,lateMinutes:facts.lateMinutes,earlyLeaveMinutes:facts.earlyLeaveMinutes});const minutes=workPolicy.recognizedMinutes;const daysRecognized=workPolicy.recognizedWorkdays;items.push(item("base_work","addition",workPolicy.workAmount,date,"기본 근무",{recognizedMinutes:minutes,recognizedDays:daysRecognized,lateMinutes:facts.lateMinutes,effectiveLateMinutes:facts.effectiveLateMinutes,earlyLeaveMinutes:facts.earlyLeaveMinutes,manualLateNormalized:facts.manualLateNormalized,overtimeCandidateMinutes:facts.overtimeCandidateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,contractRevision:contract.revision,...levelProgramSnapshot}));recognizedMinutes+=minutes;recognizedWorkdays+=daysRecognized;lateMinutes+=facts.lateMinutes;earlyLeaveMinutes+=facts.earlyLeaveMinutes;overtimeCandidateMinutes+=facts.overtimeCandidateMinutes;
    // 지각 패널티 — 두 계산기가 역할이 다르며 서로 중복 적용되지 않는다.
    //   일반 지각(manualLateNormalized=false): /admin/payroll/settings 의 minor/major tier 정책(calculateLatePenalty).
    //   수동 정상화 지각(manualLateNormalized=true): effectiveLateMinutes 를 30분 단위 올림(calculateTimePenalty).
    // 두 경로 모두 기본급은 이미 1일 전액 인정된 상태이며, grace 적용된 effectiveLateMinutes 를 입력으로 쓴다.
    if(facts.manualLateNormalized){
      const normalizedLate=calculateTimePenalty({effectiveMinutes:facts.effectiveLateMinutes,minuteRate:rate.minuteRate});
      if(normalizedLate.amount>0)items.push(item("late_deduction","deduction",normalizedLate.amount,date,"지각 패널티(수동 정상화) / Phạt đi muộn (đã chuẩn hóa)",{type:"late",latePenaltyMode:"normalized_30min",attendanceRecordId:record?.id??null,businessDate:date,rawLateMinutes:facts.rawLateMinutes,effectiveLateMinutes:facts.effectiveLateMinutes,manualLateNormalized:true,penaltyMinutes:normalizedLate.penaltyMinutes,penaltyBlockMinutes:TIME_PENALTY_BLOCK_MINUTES,lateThresholdMinutes:facts.lateThresholdMinutes,minutes:normalizedLate.penaltyMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,calculatedAmount:normalizedLate.amount,contractRevision:contract.revision,scheduleVersionId:schedule.id,scheduleRevision:schedule.revision,storeSettingsRevision:settings.revision,storePolicyRevision:settings.revision,engineVersion:PAYROLL_RUN_ENGINE_VERSION}));
    } else {
      const generalLate=calculateLatePenalty({lateMinutes:facts.effectiveLateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,thresholdMinutes:input.penaltySettings.lateMajorThresholdMinutes,minorPenaltyMinutes:input.penaltySettings.lateMinorPenaltyMinutes,majorPenaltyRateBp:input.penaltySettings.lateMajorPenaltyRateBp});
      if(generalLate.amount>0)items.push(item("late_deduction","deduction",generalLate.amount,date,"지각 패널티 / Phạt đi muộn",{type:"late",latePenaltyMode:"settings_tier",attendanceRecordId:record?.id??null,businessDate:date,lateMinutes:facts.lateMinutes,effectiveLateMinutes:facts.effectiveLateMinutes,manualLateNormalized:false,penaltyTier:generalLate.tier,thresholdMinutes:input.penaltySettings.lateMajorThresholdMinutes,minorPenaltyMinutes:input.penaltySettings.lateMinorPenaltyMinutes,majorPenaltyRateBp:input.penaltySettings.lateMajorPenaltyRateBp,minutes:facts.effectiveLateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,calculatedAmount:generalLate.amount,contractRevision:contract.revision,scheduleVersionId:schedule.id,scheduleRevision:schedule.revision,storeSettingsRevision:settings.revision,storePolicyRevision:settings.revision,engineVersion:PAYROLL_RUN_ENGINE_VERSION}));
    }
    // 조퇴 패널티: effective 조퇴분(정책 grace 공제 후)을 30분 단위 올림 → 동일 방식으로 별도 차감.
    const earlyLeavePenalty=calculateTimePenalty({effectiveMinutes:facts.earlyLeaveMinutes,minuteRate:rate.minuteRate});
    ...
    const storedRaw=Math.max(0,Number(facts.stored.workMinutes??0));const storedRecognizedMinutes=dailyContract.standardMinutesPerDay;const storedDays=1;
    ...
    if(record?.status!=="leave"&&storedRaw>0){const storedPolicy=applyPayrollWorkPolicy({contract:dailyContract,actualRecognizedMinutes:storedRecognizedMinutes,dayRate:rate.dayRate,minuteRate:rate.minuteRate,lateMinutes:Number(facts.stored.lateMinutes??0),earlyLeaveMinutes:Number(facts.stored.earlyLeaveMinutes??0)});storedAutomaticItems.push(item("base_work","addition",storedPolicy.workAmount,date,"기본 근무",{recognizedMinutes:storedPolicy.recognizedMinutes,recognizedDays:storedPolicy.recognizedWorkdays,lateMinutes:Number(facts.stored.lateMinutes??0),earlyLeaveMinutes:Number(facts.stored.earlyLeaveMinutes??0),overtimeCandidateMinutes:facts.overtimeCandidateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,contractRevision:contract.revision,attendanceSource:"stored"}));}
    const storedLateMinutes=Number(facts.stored.lateMinutes??0);
    const storedLatePenalty=facts.manualLateNormalized
      ? calculateTimePenalty({effectiveMinutes:storedLateMinutes,minuteRate:rate.minuteRate})
      : calculateLatePenalty({lateMinutes:storedLateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,thresholdMinutes:input.penaltySettings.lateMajorThresholdMinutes,minorPenaltyMinutes:input.penaltySettings.lateMinorPenaltyMinutes,majorPenaltyRateBp:input.penaltySettings.lateMajorPenaltyRateBp});
    if(record?.status!=="leave"&&storedRaw>0&&storedLatePenalty.amount>0)storedAutomaticItems.push(item("late_deduction","deduction",storedLatePenalty.amount,date,"지각 패널티 / Phạt đi muộn",{type:"late",latePenaltyMode:facts.manualLateNormalized?"normalized_30min":"settings_tier",attendanceRecordId:record?.id??null,effectiveLateMinutes:storedLateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,calculatedAmount:storedLatePenalty.amount,contractRevision:contract.revision,scheduleRevision:schedule.revision,storeSettingsRevision:settings.revision,engineVersion:PAYROLL_RUN_ENGINE_VERSION,attendanceSource:"stored"}));
    const storedEarlyLeaveMinutes=Number(facts.stored.earlyLeaveMinutes??0);const storedEarlyLeavePenalty=calculateTimePenalty({effectiveMinutes:storedEarlyLeaveMinutes,minuteRate:rate.minuteRate});if(record?.status!=="leave"&&storedRaw>0&&storedEarlyLeavePenalty.amount>0)storedAutomaticItems.push(item("early_leave_deduction","deduction",storedEarlyLeavePenalty.amount,date,"조퇴 패널티 / Phạt về sớm",{type:"early_leave",attendanceRecordId:record?.id??null,effectiveEarlyLeaveMinutes:storedEarlyLeaveMinutes,penaltyMinutes:storedEarlyLeavePenalty.penaltyMinutes,penaltyBlockMinutes:TIME_PENALTY_BLOCK_MINUTES,minutes:storedEarlyLeavePenalty.penaltyMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,calculatedAmount:storedEarlyLeavePenalty.amount,contractRevision:contract.revision,scheduleRevision:schedule.revision,storeSettingsRevision:settings.revision,engineVersion:PAYROLL_RUN_ENGINE_VERSION,attendanceSource:"stored"}));
    ...
    days.push({date,facts,sourceFact,contractRevision:contract.revision,scheduleRevision:schedule.revision,recognizedMinutes:minutes});
  }
  const deduped=[...new Map(reviews.map(entry=>[`${entry.businessDate??"none"}:${entry.warningCode}`,entry])).values()];
  const insuranceSetting=selectInsuranceSetting(input.insuranceVersionsByUser.get(user.id)??[],input.month);const insuranceSnapshot=buildEmployeeInsuranceSnapshot(insuranceSetting,input.insuranceGlobal);
  if(insuranceSnapshot.isEnrolled)items.push(item("insurance_employee_deduction","deduction",insuranceSnapshot.employeeDeductionAmount,null,"직원 보험 공제",{insuranceSettingVersionId:insuranceSnapshot.settingVersionId,insuranceSettingRevision:insuranceSnapshot.revision,effectiveMonth:insuranceSnapshot.effectiveMonth,insuranceBaseAmount:insuranceSnapshot.insuranceBaseAmount,employeeRateBp:insuranceSnapshot.employeeRateBp,employerRateBp:insuranceSnapshot.employerRateBp,employeeDeductionAmount:insuranceSnapshot.employeeDeductionAmount,employerContributionAmount:insuranceSnapshot.employerAmount}));
  return{userId:user.id,employeeName:employeeName(user),contractSnapshot:contracts,attendanceSnapshot:{month:input.month,engineVersion:PAYROLL_RUN_ENGINE_VERSION,days},insuranceSnapshot,recognizedWorkdays,recognizedMinutes,lateMinutes,earlyLeaveMinutes,overtimeCandidateMinutes,partTimeExtraWork,items,reviews:deduped};
}
...
export async function loadPayrollMonthSnapshot(month:string,options?:{calculationEndDate?:string|null;userId?:number;attendancePromise?:Promise<{data:AttendanceRow[]|null;error:unknown}>}){
  ...
  const attendanceQuery=supabaseServer.from("attendance_records").select("id,user_id,status,work_date,check_in_at,check_out_at,late_minutes,early_leave_minutes,work_minutes,note,approval_status,updated_at").gte("work_date",start).lte("work_date",attendanceEnd);
  ...
  const decisionQuery=supabaseServer.from("payroll_part_time_extra_work_decisions").select("id,attendance_record_id,decision,source_hash,decision_reason,decided_by,decided_at").eq("payroll_month",start).is("cancelled_at",null);
  const[userResult,attendanceResult,overrideResult,contractResult,scheduleResult,settingTimelineResult,decisionResult,insuranceResult,payrollSettingsResult,levelProgramResult]=await Promise.all([
  ...
  const settingRows=((settingTimelineResult.data??[]) as Record<string,unknown>[]).map(row=>({id:Number(row.id),revision:Number(row.revision),effectiveFromBusinessDate:String(row.effective_from_business_date),hours:((Array.isArray(row.store_business_hours)?row.store_business_hours:[]) as Record<string,unknown>[]).map(hour=>({weekday:Number(hour.weekday),isClosed:Boolean(hour.is_closed),openTime:hour.open_time==null?null:String(hour.open_time).slice(0,5),closeTime:hour.close_time==null?null:String(hour.close_time).slice(0,5)}))}));
  const extraWorkSettingsByDate=new Map<string,ExtraWorkStoreSetting>();for(const date of dates){const setting=settingRows.filter(row=>row.effectiveFromBusinessDate<=date).at(-1);const weekday=new Date(`${date}T00:00:00Z`).getUTCDay();const hours=setting?.hours.find(hour=>hour.weekday===weekday);if(setting&&hours&&!hours.isClosed&&hours.openTime&&hours.closeTime)extraWorkSettingsByDate.set(date,{id:setting.id,revision:setting.revision,openTime:hours.openTime,closeTime:hours.closeTime});}
  const decisionsByAttendanceId=new Map<number,PartTimeExtraWorkDecision>();for(const row of decisionResult.data??[]){const decision:PartTimeExtraWorkDecision={id:Number(row.id),attendanceRecordId:Number(row.attendance_record_id),decision:row.decision as PartTimeExtraWorkDecision["decision"],sourceHash:String(row.source_hash),decisionReason:row.decision_reason?String(row.decision_reason):null,decidedBy:Number(row.decided_by),decidedAt:String(row.decided_at)};decisionsByAttendanceId.set(decision.attendanceRecordId,decision);}
  ...
  const levelPrograms=new Map<number,EmployeeLevelProgramVersion>();for(const row of levelProgramResult.data??[]){const userId=Number(row.user_id);if(levelPrograms.has(userId))continue;levelPrograms.set(userId,{id:Number(row.id),enabled:Boolean(row.enabled),effectiveFrom:String(row.effective_from),effectiveTo:row.effective_to?String(row.effective_to):null,baseDate:row.base_date?String(row.base_date):null,baseDateMode:row.base_date_mode==="hire_date"||row.base_date_mode==="override"?row.base_date_mode:null,revision:Number(row.revision)});}const users=((userResult.data??[]) as PayrollSnapshotUserRow[]).map(user=>{const version=levelPrograms.get(Number(user.id));return version?{...user,level_program_enabled:version.enabled,level_base_date_override:version.baseDateMode==="override"?version.baseDate:null,levelProgramVersion:version}:user;});
  const input:BatchInput={month,dates,users,attendance:(attendanceResult.data??[]) as AttendanceRow[],lateNormalizedRecordIds:new Set((overrideResult.data??[]).map(row=>Number(row.attendance_record_id))),contracts:(contractResult.data??[]).map(row=>mapContract(row as Record<string,unknown>)),schedules:(scheduleResult.data??[]).map(row=>mapSchedule(row as Record<string,unknown>)),settingsByDate,extraWorkSettingsByDate,decisionsByAttendanceId,insuranceVersionsByUser,insuranceGlobal,penaltySettings};
