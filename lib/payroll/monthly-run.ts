...
import { applyPayrollWorkPolicy } from "./work-policy";
...
export const PAYROLL_RUN_ENGINE_VERSION = "monthly-payroll-v5";
...
type AutoCategory="base_work"|"paid_leave"|"overtime"|"late_deduction"|"early_leave_deduction"|"insurance_employee_deduction";
...
type BatchInput={month:string;users:PayrollSnapshotUserRow[];attendance:AttendanceRow[];contracts:PayrollContract[];schedules:WorkScheduleVersion[];settingsByDate:Map<string,{revision:number|null;lateGraceMinutes:number;earlyLeaveGraceMinutes:number}>;insuranceVersionsByUser:Map<number,PayrollInsuranceSettingVersion[]>;insuranceGlobal:PayrollInsuranceGlobalSettings};
...
function rates(contract:PayrollContract,salaryBase=contract.baseSalary){const dayRate=contract.payType==="monthly"?(contract.standardWorkdays?salaryBase/contract.standardWorkdays:0):contract.payType==="daily"?contract.baseSalary:(contract.baseSalary/60)*contract.standardMinutesPerDay;return{dayRate,minuteRate:contract.payType==="hourly"?contract.baseSalary/60:dayRate/contract.standardMinutesPerDay};}
...
function calculateEmployee(user:PayrollSnapshotUserRow,records:AttendanceRow[],input:BatchInput):PayrollRunEmployeeInput{
  ...
  for(const record of eligibleRecords){const date=record.work_date;const contractMatches=activeOn(contracts,date);const scheduleMatches=activeOn(schedules,date);const contract=contractMatches.length===1?contractMatches[0]:null;const schedule=scheduleMatches.length===1?scheduleMatches[0]:null;const settings=input.settingsByDate.get(date)??{revision:null,lateGraceMinutes:0,earlyLeaveGraceMinutes:0};
    ...
    const levelInfo=getEmployeeLevelInfo(user,date);const compensation=calculateCombinedSalary(contract,levelInfo);if(compensation.combinedSalary===null){reviews.push(review("EMPLOYEE_LEVEL_BASE_DATE_REQUIRED",date,{contractRevision:contract.revision}));continue;}const rate=rates(contract,compensation.combinedSalary);
    ...
    let actualRecognizedMinutes=facts.scheduledOverlapMinutes;if(contract.payType!=="monthly"&&contract.calculationBasis==="hour")actualRecognizedMinutes=roundMinutes(actualRecognizedMinutes,contract.timeBlockMinutes,contract.roundingMode);
    ...
    if(workPolicy.automaticLatePenalty>0)items.push(item("late_deduction","deduction",workPolicy.automaticLatePenalty,date,"지각 공제",{minutes:facts.lateMinutes,minuteRate:rate.minuteRate,attendanceRecordId:record.id}));else if(workPolicy.lateRequiresReview)reviews.push(review("LATE_POLICY_REVIEW",date,{minutes:facts.lateMinutes,minuteRate:rate.minuteRate,suggestedDeduction:vnd(rate.minuteRate*facts.lateMinutes),attendanceRecordId:record.id}));
    if(workPolicy.automaticEarlyLeavePenalty>0)items.push(item("early_leave_deduction","deduction",workPolicy.automaticEarlyLeavePenalty,date,"조퇴 공제",{minutes:facts.earlyLeaveMinutes,minuteRate:rate.minuteRate,attendanceRecordId:record.id}));else if(workPolicy.earlyLeaveRequiresReview)reviews.push(review("EARLY_LEAVE_POLICY_REVIEW",date,{minutes:facts.earlyLeaveMinutes,minuteRate:rate.minuteRate,suggestedDeduction:vnd(rate.minuteRate*facts.earlyLeaveMinutes),attendanceRecordId:record.id}));
...
export async function loadPayrollMonthSnapshot(month:string,options?:{calculationEndDate?:string|null}){
  const monthDates=payrollMonthDates(month);const start=monthDates[0];const calculationEndDate=options?.calculationEndDate;const dates=options&&"calculationEndDate" in options?(calculationEndDate?monthDates.filter(date=>date<=calculationEndDate):[]):monthDates;const dayBeforeMonth=new Date(`${start}T00:00:00Z`);dayBeforeMonth.setUTCDate(dayBeforeMonth.getUTCDate()-1);const attendanceEnd=dates.at(-1)??dayBeforeMonth.toISOString().slice(0,10);const nextMonth=new Date(`${start}T00:00:00Z`);nextMonth.setUTCMonth(nextMonth.getUTCMonth()+1);const endExclusive=nextMonth.toISOString().slice(0,10);
  const[userResult,attendanceResult,contractResult,scheduleResult,settingsResults,insuranceResult,payrollSettingsResult]=await Promise.all([
    ...
    supabaseServer.from("payroll_settings").select("employee_insurance_rate_bp,employer_insurance_rate_bp,director_insurance_enabled,director_insurance_base_amount,director_insurance_rate_bp,updated_at").eq("id",1).single(),
  ]);
  if(userResult.error||attendanceResult.error||contractResult.error||scheduleResult.error||settingsResults.some(result=>result.error)||insuranceResult.error||payrollSettingsResult.error)throw new Error("PAYROLL_MONTH_SNAPSHOT_READ_FAILED");
  ...
  const input:BatchInput={month,users:(userResult.data??[]) as PayrollSnapshotUserRow[],attendance:(attendanceResult.data??[]) as AttendanceRow[],contracts:(contractResult.data??[]).map(row=>mapContract(row as Record<string,unknown>)),schedules:(scheduleResult.data??[]).map(row=>mapSchedule(row as Record<string,unknown>)),settingsByDate,insuranceVersionsByUser,insuranceGlobal};
  ...
  return{employees:calculatePayrollBatch(input),context:{users:input.users,contracts:input.contracts},sourceSnapshot:{engineVersion:PAYROLL_RUN_ENGINE_VERSION,calculatedAt:new Date().toISOString(),attendanceRecordIds:input.attendance.map(row=>row.id),contractRevisions:[...new Set(input.contracts.map(row=>row.revision))],scheduleRevisions:[...new Set(input.schedules.map(row=>row.revision))],storeSettingRevisions:[...new Set([...settingsByDate.values()].map(value=>value.revision).filter(value=>value!==null))],insuranceSettings:insuranceSettingsSnapshot}};
}
