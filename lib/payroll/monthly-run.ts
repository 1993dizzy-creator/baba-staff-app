...
function calculateEmployee(user:PayrollSnapshotUserRow,records:AttendanceRow[],input:BatchInput):PayrollRunEmployeeInput{
  ...
  for(const date of eligibleDates){const record=recordsByDate.get(date)??null;const contractMatches=activeOn(contracts,date);const scheduleMatches=activeOn(schedules,date);const contract=contractMatches.length===1?contractMatches[0]:null;const schedule=scheduleMatches.length===1?scheduleMatches[0]:null;if(!record&&!schedule)continue;const settings=input.settingsByDate.get(date)??{revision:null,lateGraceMinutes:0,earlyLeaveGraceMinutes:0};
    ...
    const scheduleMinutes=facts.scheduledMinutes;if(scheduleMinutes===null){days.push({date,facts,contractRevision:contract.revision,scheduleRevision:schedule.revision});continue;}const dailyContract={...contract,standardMinutesPerDay:scheduleMinutes};
    ...
    const storedRaw=Math.max(0,Number(facts.stored.workMinutes??0));const storedRecognizedMinutes=facts.manualLateNormalized?facts.scheduledOverlapMinutes??storedRaw:contract.payType==="monthly"?dailyContract.standardMinutesPerDay:contract.calculationBasis==="hour"?roundMinutes(storedRaw,contract.timeBlockMinutes,contract.roundingMode):storedRaw;const storedDays=storedRecognizedMinutes/dailyContract.standardMinutesPerDay;
    ...
    const storedLateMinutes=Number(facts.stored.lateMinutes??0);const storedLatePenalty=calculateLatePenalty({lateMinutes:storedLateMinutes,minuteRate:rate.minuteRate,dayRate:rate.dayRate,thresholdMinutes:input.penaltySettings.lateMajorThresholdMinutes,minorPenaltyMinutes:input.penaltySettings.lateMinorPenaltyMinutes,majorPenaltyRateBp:input.penaltySettings.lateMajorPenaltyRateBp});if(record?.status!=="leave"&&storedRaw>0&&storedLatePenalty.amount>0)storedAutomaticItems.push(item("late_deduction","deduction",storedLatePenalty.amount,date,"지각 패널티",{attendanceRecordId:record?.id??null,lateMinutes:storedLateMinutes,penaltyTier:storedLatePenalty.tier,thresholdMinutes:input.penaltySettings.lateMajorThresholdMinutes,minorPenaltyMinutes:input.penaltySettings.lateMinorPenaltyMinutes,majorPenaltyRateBp:input.penaltySettings.lateMajorPenaltyRateBp,minuteRate:rate.minuteRate,dayRate:rate.dayRate,calculatedAmount:storedLatePenalty.amount,contractRevision:contract.revision,scheduleRevision:schedule.revision,storeSettingsRevision:settings.revision,engineVersion:PAYROLL_RUN_ENGINE_VERSION,attendanceSource:"stored"}));
...
export async function loadPayrollMonthSnapshot(month:string,options?:{calculationEndDate?:string|null;userId?:number;attendancePromise?:Promise<{data:AttendanceRow[]|null;error:unknown}>}){
  ...
  const input:BatchInput={month,dates,users,attendance:(attendanceResult.data??[]) as AttendanceRow[],lateNormalizedRecordIds:new Set((overrideResult.data??[]).map(row=>Number(row.attendance_record_id))),contracts:(contractResult.data??[]).map(row=>mapContract(row as Record<string,unknown>)),schedules:(scheduleResult.data??[]).map(row=>mapSchedule(row as Record<string,unknown>)),settingsByDate,insuranceVersionsByUser,insuranceGlobal,penaltySettings};
