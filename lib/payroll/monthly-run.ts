...
export const PAYROLL_RUN_ENGINE_VERSION = "monthly-payroll-v4";
...
type BatchInput={month:string;users:PayrollSnapshotUserRow[];attendance:AttendanceRow[];contracts:PayrollContract[];schedules:WorkScheduleVersion[];settingsByDate:Map<string,{revision:number|null;lateGraceMinutes:number;earlyLeaveGraceMinutes:number}>};
...
export async function loadPayrollMonthSnapshot(month:string,options?:{calculationEndDate?:string|null}){
  ...
  const input:BatchInput={month,users:(userResult.data??[]) as PayrollSnapshotUserRow[],attendance:(attendanceResult.data??[]) as AttendanceRow[],contracts:(contractResult.data??[]).map(row=>mapContract(row as Record<string,unknown>)),schedules:(scheduleResult.data??[]).map(row=>mapSchedule(row as Record<string,unknown>)),settingsByDate};
