...
export async function loadOwnerDashboard(throughMonth:string,{investmentView="month_end"}:{investmentView?:OwnerInvestmentView}={}){
 ...
 const[capacityResult,participantResult,investmentResult,settlementResult,allocationResult,policyResult,accountResult,userResult,closureResult,settingsResult]=await Promise.all([
  ...
  supabaseServer.from("ledger_owner_settlements").select("id,through_month,status,confirmed_pool,policy_snapshot,confirmed_at").lte("through_month",monthDate).order("through_month",{ascending:false}),
 ...
 const participants=participantResult.data??[],allParticipantResult=await supabaseServer.from("ledger_owner_participants").select("id,user_id");if(allParticipantResult.error)throw allParticipantResult.error;const allParticipantUser=new Map((allParticipantResult.data??[]).map(row=>[Number(row.id),Number(row.user_id)]));
 ...
 return{capacity:capacityResult.data,participants,owners,users:userResult.data??[],accounts:accountResult.data??[],settlements:(settlementResult.data??[]).map(row=>({...row,allocations:visibleAllocations.filter(item=>Number(item.settlement_id)===Number(row.id))})),policy,previewLines,closedMonths:(closureResult.data??[]).map(row=>String(row.month).slice(0,7)),settings:settingsResult.data,participantUser:Object.fromEntries(allParticipantUser)};
}
