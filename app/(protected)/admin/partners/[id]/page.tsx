...
export default function PartnerDetailPage() {
  ...
  const [partner, setPartner] = useState<Partner | null>(null); const [ledgerParties, setLedgerParties] = useState<LedgerParty[]>([]); const [fundAccounts, setFundAccounts] = useState<FundAccount[]>([]); const [linkedInventory, setLinkedInventory] = useState<LinkedInventory[]>([]); const [message, setMessage] = useState(""); const [error, setError] = useState("");
  const load = useCallback(async () => { const response = await fetch(`/api/admin/partners/${params.id}`, { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.code); setPartner({ ...body.partner, ledgerPartyId: body.partner.ledgerParty?.id ?? null }); setLedgerParties(body.ledgerParties); setFundAccounts(body.fundAccounts); setLinkedInventory(body.linkedInventory); }, [params.id]);
  ...
  return <Container><main className={styles.page}><Link className={styles.backLink} href="/admin/partners/info">← {lang === "vi" ? "Thông tin" : "정보"}</Link><header className={styles.detailTop}><h1>{partner?.name ?? t.title}</h1></header>{message ? <p className={styles.notice} role="status">{message}</p> : null}{error ? <p className={styles.error} role="alert">{error}</p> : null}{partner ? <section className={styles.panel}><h2>{t.edit}</h2><PartnerForm key={`${partner.id}-${partner.name}-${partner.isActive}`} lang={lang} initial={partner} ledgerParties={ledgerParties} fundAccounts={fundAccounts} onSubmit={update} /></section> : null}<section className={styles.panel}><h2>{t.supply} {linkedInventory.length}</h2>{linkedInventory.length === 0 ? <p className={styles.muted}>{t.noSupply}</p> : <div className={styles.inventoryList}>{linkedInventory.map(item => <article key={item.id}><div><strong>{lang === "vi" ? item.itemNameVi || item.itemName : item.itemName || item.itemNameVi}</strong><span>{item.part || "-"} · {lang === "vi" ? item.categoryVi || item.category : item.category || item.categoryVi || "-"}</span></div><div><strong>{item.purchasePrice === null ? "-" : `${new Intl.NumberFormat("vi-VN").format(Number(item.purchasePrice))} ₫`}</strong><span>{item.isActive ? t.active : t.inactive}</span></div></article>)}</div>}</section></main></Container>;
}
