...
export default function LedgerSettingsPage() {
  ...
  const [party, setParty] = useState({ name: "", categoryId: "", termDays: "", memo: "" });
  ...
  async function createParty(event: FormEvent) { event.preventDefault(); await mutate("/api/admin/ledger/parties", { name: party.name, defaultCategoryId: party.categoryId ? Number(party.categoryId) : null, defaultPaymentTermDays: party.termDays ? Number(party.termDays) : null, memo: party.memo }); setParty({ name: "", categoryId: "", termDays: "", memo: "" }); }
  ...
  return <Container><main className={styles.page}>
    ...
    <div className={styles.settingsGrid}>
      ...
      <details className={styles.settingsSection}><summary>거래처</summary><p className={styles.sectionDescription}>외상 및 반복 거래에 사용할 기본정보를 관리합니다.</p><div className={styles.chips}>{ledger?.parties.map(row => <span className={styles.chip} key={row.id}>{row.name}</span>)}</div><form className={styles.form} onSubmit={createParty}><label>거래처명<input required className={styles.input} value={party.name} onChange={event => setParty({ ...party, name: event.target.value })} /></label><label>기본 비용 카테고리<select className={styles.input} value={party.categoryId} onChange={event => setParty({ ...party, categoryId: event.target.value })}><option value="">설정 안 함</option>{expenseCategories.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label>기본 결제기한(일)<input className={styles.input} inputMode="numeric" value={party.termDays} onChange={event => setParty({ ...party, termDays: event.target.value })} /></label><label>메모<input className={styles.input} value={party.memo} onChange={event => setParty({ ...party, memo: event.target.value })} /></label><button className={styles.primary} disabled={working}>거래처 추가</button></form></details>
