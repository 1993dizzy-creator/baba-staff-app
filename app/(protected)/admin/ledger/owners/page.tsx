...
import { allocateOwnerRecoveryPool } from "@/lib/ledger/owner-recovery-core";
import styles from "./owners.module.css";

type Owner = { participantId:number; userId:number; name:string; sortOrder:number; cumulativeInvested:number; recoveryAllocated:number; recoveryPaid:number; unrecoveredForAllocation:number; cashUnrecovered:number; pureProfitAllocated:number; pureProfitPaid:number; unpaidSettlement:number };
...
type Settlement = { id:number; through_month:string; settlement_type:"capital_recovery"|"profit_distribution"; confirmed_at:string|null; status:string; confirmed_pool:number; allocations:Allocation[] };
type RecoveryCapacity = { totalInvested:number; totalCashUnrecoveredInvestment:number; totalUnallocatedUnrecoveredInvestment:number; safeCashCapacity:number; recommendedMaxRecovery:number };
type Dashboard = { recoveryCapacity:RecoveryCapacity; owners:Owner[]; accounts:Account[]; settlements:Settlement[]; policy:{lines:Array<{participant_id:number;settlement_rate:string}>}|null; participantUser:Record<string,number> };
type Sheet = "owner"|"investment"|"recovery"|"recent"|null;
...
const settlementLabel=(row:Settlement)=>row.settlement_type==="capital_recovery"?"투자금 회수":"이익 정산";
const settlementDate=(row:Settlement)=>row.settlement_type==="capital_recovery"&&row.confirmed_at?new Intl.DateTimeFormat("sv-SE",{timeZone:"Asia/Ho_Chi_Minh",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(row.confirmed_at)):monthLabel(String(row.through_month).slice(0,7));
...
export default function OwnerSettlementsPage(){
  ...
  function openRecovery(){setPool("");setFormError("");setSheet("recovery")}
  ...
  async function confirmRecovery(){if(await mutate({action:"confirm_recovery",requestedPool:pool}))closeSheet()}
  ...
  const owners=useMemo(()=>currentData?.owners??[],[currentData]);
  ...
  const recoveryCapacity=currentData?.recoveryCapacity;
  const recommendedMax=Number(recoveryCapacity?.recommendedMaxRecovery??0);
  const canConfirm=validAmount(pool)&&Number(pool)>0&&Number(pool)<=recommendedMax&&!busy;
  const preview=useMemo(()=>{
    if(!canConfirm)return [];
    try{return allocateOwnerRecoveryPool(pool,owners.map(owner=>({userId:owner.userId,participantId:owner.participantId,unrecoveredForAllocation:owner.unrecoveredForAllocation}))) }
    catch{return []}
  },[canConfirm,owners,pool]);
  ...
  return <Container noPaddingTop><main className={styles.page}>
    ...
    <section className={styles.section}><h2>사장별 현황</h2>{owners.length?<div className={styles.ownerList}>{owners.map(owner=>{
      ...
      return <button type="button" className={styles.ownerCard} key={owner.participantId} onClick={()=>{setOwnerId(owner.participantId);setSheet("owner")}}><span className={styles.ownerName}>{owner.name}<i aria-hidden>›</i></span><span>누적 투자금 <b>{money(owner.cumulativeInvested)}</b></span><span>회수 완료 <b>{money(owner.recoveryPaid)}</b></span><span>미회수 <b>{money(owner.cashUnrecovered)}</b></span><span>이익 배분 비율 <b>{rate?percent(rate):"-"}</b></span></button>})}</div>:<p className={styles.empty}>설정된 투자자가 없습니다.</p>}</section>
    <div className={styles.actions}><button type="button" className={styles.primary} onClick={openInvestment} disabled={!owners.length}>+ 투자금 등록</button><button type="button" className={styles.secondary} onClick={openRecovery} disabled={!recoveryCapacity||Number(recoveryCapacity.totalInvested)<=0||Number(recoveryCapacity.totalCashUnrecoveredInvestment)<=0}>투자금 회수</button></div>
    {recoveryCapacity&&Number(recoveryCapacity.totalInvested)===0?<p className={styles.empty}>먼저 초기 투자금을 등록해주세요.</p>:null}
    {recoveryCapacity&&Number(recoveryCapacity.totalInvested)>0&&Number(recoveryCapacity.totalCashUnrecoveredInvestment)===0?<p className={styles.notice}>투자금 회수가 완료되었습니다. 이후 이익 배분은 별도 정산합니다.</p>:null}
    <p className={styles.settingsHint}>투자자 구성과 이익 배분 비율은 <Link href="/admin/ledger/settings">장부설정</Link>에서 관리합니다.</p>
    ...
    {sheet==="recovery"?<BarSheet kind="bottom" topAligned comfortableTop title="투자금 회수" closeLabel="닫기" saving={busy} onClose={closeSheet} footer={<button className={styles.primary} type="button" disabled={!canConfirm} onClick={()=>void confirmRecovery()}>회수 확정</button>}><div className={styles.form}>
      <Figure label="현재 미회수 투자금" value={recoveryCapacity?.totalUnallocatedUnrecoveredInvestment??0}/>
      <Figure label="안전 현금" value={recoveryCapacity?.safeCashCapacity??0}/>
      <Figure label="회수 가능 최대" value={recommendedMax}/>
      <label>이번 회수 금액<input className={styles.input} inputMode="decimal" value={pool} onChange={event=>setPool(event.target.value)} placeholder="금액 입력"/></label>
      {preview.length?<div className={styles.preview}><h3>예상 배분 <small>미회수 원금 비례</small></h3>{preview.map(line=><div key={line.userId}><span>{owners.find(owner=>owner.userId===line.userId)?.name??"투자자"}<small>미회수 {money(line.unrecoveredForAllocation)}</small></span><strong>이번 회수 {money(line.assignedAmount)}</strong></div>)}</div>:null}
      {formError?<p role="alert" className={styles.error}>{formError}</p>:null}
    </div></BarSheet>:null}

    {sheet==="recent"&&selectedSettlement?<BarSheet kind="bottom" title={`${settlementDate(selectedSettlement)} ${settlementLabel(selectedSettlement)}`} closeLabel="닫기" saving={busy} onClose={closeSheet} footer={<button className={styles.secondary} type="button" onClick={closeSheet}>닫기</button>}><div className={styles.form}><Figure label="확정 금액" value={selectedSettlement.confirmed_pool}/>
