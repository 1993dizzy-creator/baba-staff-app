...
export async function PATCH(
  ...
  try {
    ...
    if (!inventoryItemId || !quantityPerPosUnit) {
      return NextResponse.json(
        { ok: false, error: "inventoryItemId and quantityPerPosUnit are required." },
        { status: 400 }
      );
    }
    ...
    if (!data) {
      return NextResponse.json(
        { ok: false, error: "Keg tracking mapping was not found." },
        { status: 404 }
      );
    }
    ...
    console.error("[ADMIN_KEG_TRACKING_MAPPINGS_PATCH_ERROR]", error);
    return NextResponse.json(
      {
        ok: false,
...
export async function DELETE(
  ...
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as JsonObject;
    const actorUsername =
      getString(body.actorUsername) || getString(searchParams.get("actorUsername"));
    const actor = await getMappingAdminActor(actorUsername);
    ...
    return NextResponse.json({ ok: true, mapping: serializeMapping(data) });
  } catch (error) {
    console.error("[ADMIN_KEG_TRACKING_MAPPINGS_DELETE_ERROR]", error);
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Failed to disable keg tracking mapping.",
      },
      { status: 500 }
    );
  }
