...
export async function GET() {
  ...
  const [payableResult,paymentResult,bridgeResult,partnerResult]=await Promise.all([supabaseServer.from("ledger_payables")
    ...
    .neq("status", "cancelled").order("created_at", { ascending: false }),supabaseServer.from("ledger_transactions").select("party_id,business_date").eq("type","payable_payment").eq("status","confirmed").order("business_date",{ascending:false}),supabaseServer.from("business_partner_ledger_parties").select("business_partner_id,ledger_party_id"),supabaseServer.from("business_partners").select("id,partner_type")]);
  const loadError=payableResult.error??paymentResult.error??bridgeResult.error??partnerResult.error;
  ...
  const map=new Map<number,{partyId:number;partyName:string;partnerType:string|null;outstandingAmount:number;openCount:number;oldestDate:string;nearestDueDate:string|null;recentPaymentDate:string|null}>();
  for(const row of payables){const partyRelation=row.party as unknown as {name:string}|null;const partyId=Number(row.party_id);const expense=row.expense as unknown as {business_date:string}|null;const businessPartnerId=businessPartnerByLedgerParty.get(partyId);const current=map.get(partyId)??{partyId,partyName:partyRelation?.name??"-",partnerType:businessPartnerId===undefined?null:partnerTypeByBusinessPartner.get(businessPartnerId)??null,outstandingAmount:0,openCount:0,oldestDate:expense?.business_date??"",nearestDueDate:null,recentPaymentDate:recent.get(partyId)??null};current.outstandingAmount+=row.outstandingAmount;current.openCount+=1;if(expense?.business_date&&(!current.oldestDate||expense.business_date<current.oldestDate))current.oldestDate=expense.business_date;if(row.due_date&&(!current.nearestDueDate||row.due_date<current.nearestDueDate))current.nearestDueDate=row.due_date;map.set(partyId,current)}
