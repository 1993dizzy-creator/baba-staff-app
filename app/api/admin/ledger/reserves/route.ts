import { reserveCurrentAmount } from "@/lib/ledger/reserve-balances";
...
export async function GET() {
  ...
  try {
    const [planResult, accountResult, movementResult] = await Promise.all([
      ...
      supabaseServer.from("ledger_fund_accounts").select("id,type,code,is_active,is_business_fund").eq("is_active", true),
    ...
    const plans = (planResult.data ?? []).map((plan) => {
      ...
      return { ...plan, currentAmount, remainingAmount, remainingMonths, monthlyRequired };
    });
    ...
    return ledgerJson({ ok: true, plans, liquidFunds, activeReserve, freeCash: liquidFunds - activeReserve });
  } catch (error) {
