...
export async function POST(request: Request) {
  ...
  return error?payrollJson({ok:false,code:"PAYROLL_ADJUSTMENT_CREATE_FAILED"},500):payrollJson({ok:true,adjustment:data},201);
}
...
export async function PATCH(request: Request) {
  ...
  if(error)return payrollJson({ok:false,code:"PAYROLL_ADJUSTMENT_CANCEL_FAILED"},500);return data?payrollJson({ok:true,adjustment:data}):payrollJson({ok:false,code:"ADJUSTMENT_NOT_ACTIVE"},409);
}
