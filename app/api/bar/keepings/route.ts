...
export async function POST(request: NextRequest) {
  ...
  try{
    ...
    const form=await request.formData();const customerName=cleanText(form.get("customerName"),120,true),contact=cleanText(form.get("customerContact"),120),identifier=cleanText(form.get("customerIdentifier"),120),note=cleanText(form.get("note"),3000),zone=cleanText(form.get("zoneCode"),8,true),percent=cleanPercent(form.get("remainingPercent")),storedAt=cleanDate(form.get("storedAt"),true);
    const liquor=await resolveKeepingLiquor(form.get("liquorSource"),form.get("inventoryItemId"),form.get("liquorName"));
    const detail=form.get("image"),thumb=form.get("thumbnail");if(customerName===undefined||contact===undefined||identifier===undefined||!liquor||note===undefined||!zone||percent===undefined||!storedAt||!(detail instanceof File)||!(thumb instanceof File))return bad("Invalid keeping data");
    if(!await validKeepingZone(zone))return bad("Invalid keeping zone");paths=await uploadKeepingFiles(detail,thumb);
    const {data,error}=await supabaseServer.rpc("bar_create_keeping",{p_customer_name:customerName,p_customer_contact:contact,p_customer_identifier:identifier,p_liquor_source:liquor.liquorSource,p_inventory_item_id:liquor.inventoryItemId,p_liquor_name:liquor.liquorName,p_note:note,p_zone_code:zone,p_remaining_percent:percent,p_image_path:paths.imagePath,p_thumbnail_path:paths.thumbnailPath,p_stored_at:storedAt,p_expires_at:null,p_actor_user_id:actor.id});if(error)throw error;
