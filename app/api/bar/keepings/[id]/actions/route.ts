...
async function validate(action:string,raw:Record<string,unknown>,currentZone:string,allowClosed:boolean){
  ...
  if(action==="reactivate"){const zone_code=cleanText(raw.zoneCode,8,true),remaining_percent=cleanPercent(raw.remainingPercent),stored_at=cleanDate(raw.storedAt,true),reason=cleanText(raw.reason,500,true);if(!zone_code||remaining_percent===undefined||!stored_at||!reason||!await validKeepingZone(zone_code))return null;return{zone_code,remaining_percent,stored_at,reason};}
  return null;
}
