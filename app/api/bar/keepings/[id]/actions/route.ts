...
import { KEEPING_REACTIVATE_RPC, parseReactivateActionForm } from "@/lib/bar/keeping-reactivate-request";
...
export async function POST(request:NextRequest,context:Context){
  ...
  try{const {actor,response}=await getBarServerActor();if(response||!actor)return response;if(!canManageBarKeeping(actor))return forbidden();
    ...
    let version:number;let payload:Record<string,unknown>;let detail:FormDataEntryValue|null;let thumb:FormDataEntryValue|null;let mutationRpc:string=KEEPING_REACTIVATE_RPC;
    if(action==="reactivate"){
      const parsed=await parseReactivateActionForm(form,validKeepingZone);
      if(!parsed.ok){console.warn("[KEEPING_ACTION_VALIDATION]",{keepingId:id,failedField:parsed.field,...parsed.diagnostic});return bad(`REACTIVATE_${fieldCode(parsed.field)}`,`REACTIVATE_${fieldCode(parsed.field)}`);}
      version=parsed.version;payload=parsed.payload;detail=parsed.detail;thumb=parsed.thumbnail;mutationRpc=parsed.rpc;
    }else{
      const cleanedVersion=cleanVersion(form.get("version"));if(!cleanedVersion)return bad("Invalid action", "INVALID_VERSION");version=cleanedVersion;
      let raw:Record<string,unknown>;try{raw=JSON.parse(String(form.get("payload")??"{}"));}catch{return bad("Invalid payload","INVALID_PAYLOAD");}
      const validated=await validate(action,raw,current.zone_code,canEditClosedBarKeeping(actor));if(!validated)return bad("Invalid action data");payload=validated;detail=form.get("image");thumb=form.get("thumbnail");
    }
    const hasFiles=detail instanceof File&&detail.size>0||thumb instanceof File&&thumb.size>0;
...
const bad=(error:string,code="INVALID_INPUT")=>NextResponse.json({ok:false,error,code},{status:400});const forbidden=()=>NextResponse.json({ok:false,error:"Forbidden"},{status:403});const conflict=(error:string)=>NextResponse.json({ok:false,error,code:"INVALID_STATE"},{status:409});
const fieldCode=(field:string)=>field.replace(/[A-Z]/g,letter=>`_${letter}`).toUpperCase();
