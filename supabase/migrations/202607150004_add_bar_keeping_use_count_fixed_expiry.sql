-- Count actual keeping-use actions and enforce a three-calendar-month storage period.
-- Apply after 202607150003_add_bar_keeping_liquor_source.sql.

alter table public.bar_keepings
  add column use_count integer not null default 0,
  add constraint bar_keepings_use_count_check check (use_count >= 0);

update public.bar_keepings as k
set use_count = (
  select count(*)::integer
  from public.bar_activity_logs as l
  where l.entity_type = 'keeping'
    and l.entity_id = k.id
    and l.action_type = 'keeping_used'
);

update public.bar_keepings
set expires_at = (stored_at + interval '3 months')::date
where status = 'active';
...
begin
  ...
  insert into public.bar_keepings(customer_name,customer_identifier,liquor_name,liquor_source,inventory_item_id,note,zone_code,remaining_percent,
    image_path,thumbnail_path,image_updated_at,stored_at,expires_at,use_count,created_by_user_id,updated_by_user_id,created_at,updated_at)
  values (btrim(p_customer_name),nullif(btrim(p_customer_identifier),''),v_liquor_name,p_liquor_source,p_inventory_item_id,
    nullif(btrim(p_note),''),p_zone_code,p_remaining_percent,p_image_path,p_thumbnail_path,v_now,p_stored_at,
    (p_stored_at + interval '3 months')::date,0,p_actor_user_id,p_actor_user_id,v_now,v_now) returning * into v_row;
  ...
  values ('keeping',v_row.id,left(v_row.customer_name || ' · ' || v_row.liquor_name,240),'keeping_created',null,
    jsonb_build_object('customer_name',v_row.customer_name,'liquor_name',v_row.liquor_name,'liquor_source',v_row.liquor_source,
      ...
      'stored_at',v_row.stored_at,'expires_at',v_row.expires_at,'use_count',v_row.use_count),
...
declare
  v_result jsonb; v_payload jsonb := p_payload; v_name text; v_source text; v_item_id bigint;
  v_log_id bigint; v_stored_at date; v_use_count integer; v_action_type text;
begin
  if p_action = 'update' then
    v_stored_at := (p_payload->>'stored_at')::date;
  ...
  if p_action = 'update' then
    ...
    v_action_type := 'keeping_used';
  elsif p_action = 'reactivate' then
    update public.bar_keepings
      set stored_at=v_stored_at,expires_at=(v_stored_at + interval '3 months')::date
      where id=p_id;
    v_action_type := 'keeping_reactivated';
  end if;

  if v_action_type is not null then
    select l.id into v_log_id from public.bar_activity_logs l
      where l.entity_type='keeping' and l.entity_id=p_id and l.action_type=v_action_type and l.actor_user_id=p_actor_user_id
  ...
  if v_log_id is not null and p_action = 'update' then
    update public.bar_activity_logs set after_data=after_data || jsonb_build_object(
      'liquor_source',v_source,'inventory_item_id',v_item_id,'liquor_name',v_name,
      'stored_at',v_stored_at,'expires_at',(v_stored_at + interval '3 months')::date) where id=v_log_id;
