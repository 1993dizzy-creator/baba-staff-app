...
create unique index if not exists pos_inventory_deductions_success_reversal_uidx
  on public.pos_inventory_deductions (reversal_of_deduction_id)
  where operation_type = 'revert'
    and reversal_of_deduction_id is not null
    and (
      status in ('applied', 'success', 'reverted')
      or applied_at is not null
      or inventory_log_id is not null
    );
...
begin
  ...
  delete from pg_temp.sales_reprocess_active_deductions;
