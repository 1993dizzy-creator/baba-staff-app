create or replace function public.bar_delete_keeping_v2(
  p_id bigint,
...
begin
  ...
  if v_old.status not in ('active', 'closed') then
...
revoke all on function public.bar_delete_keeping_v2(bigint, integer, bigint)
  from public, anon, authenticated;
grant execute on function public.bar_delete_keeping_v2(bigint, integer, bigint)
  to service_role;

comment on function public.bar_delete_keeping_v2(bigint, integer, bigint) is
  'Owner/master-only versioned hard delete for active or closed BAR keepings with a minimal audit snapshot.';
