...
-- Preflight: this migration is intentionally coupled to the production schema
-- inspected on 2026-08-12. Fail before changing anything if that shape drifted.
do $$
declare
...
begin
  if to_regclass('public.users') is null
    ...
    or to_regclass('public.attendance_record_audit_logs') is null
    or to_regclass('public.payroll_payment_batches') is null
    or to_regclass('public.payroll_employee_payments') is null
    or to_regclass('public.employee_work_schedule_versions') is null
    or to_regclass('public.store_setting_versions') is null
    or to_regclass('public.store_business_hours') is null
    or to_regclass('public.store_holidays') is null
    or to_regclass('public.store_holiday_operation_policies') is null
    or to_regprocedure('public.store_business_date_for_timestamp_v1(timestamp with time zone)') is null
  then
    raise exception 'preflight failed: unauthorized absence dependencies are missing';
  end if;
  ...
  if exists (
    ...
    raise exception 'preflight failed: attendance_records_status_check already exists';
  end if;

  if not exists (
    select 1 from pg_indexes
    where schemaname = 'public' and tablename = 'attendance_records'
      and indexname = 'attendance_records_user_id_work_date_key'
      and indexdef like 'CREATE UNIQUE INDEX% (user_id, work_date)'
  ) then
    raise exception 'preflight failed: attendance user/work-date uniqueness is missing';
  end if;

  if exists (
    select 1 from public.attendance_records
    where status not in ('before', 'working', 'done', 'late', 'early_leave', 'leave')
  ) then
    raise exception 'preflight failed: unexpected attendance status exists';
  end if;

  select pg_get_constraintdef(oid) into v_audit_action_definition
  from pg_constraint
  where conrelid = 'public.attendance_record_audit_logs'::regclass
    and conname = 'attendance_record_audit_logs_action_check';

  if v_audit_action_definition is distinct from
    'CHECK ((action = ANY (ARRAY[''manual_update''::text, ''normalize_late''::text, ''normalize_early_leave''::text, ''auto_close''::text, ''policy_recalculation''::text, ''cancel_check_in''::text, ''cancel_check_out''::text, ''cancel_leave''::text])))'
  then
    raise exception 'preflight failed: unexpected attendance audit action constraint';
  end if;

  if to_regprocedure('public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)') is not null then
    raise exception 'preflight failed: attendance_admin_unauthorized_absence_v1 already exists';
  end if;
end
$$;
...
alter table public.attendance_records
  add constraint attendance_records_status_check check (
    status in ('before', 'working', 'done', 'late', 'early_leave', 'leave', 'unauthorized_absence')
  );
...
alter table public.attendance_record_audit_logs
  add constraint attendance_record_audit_logs_action_check check (
    action in (
      'manual_update', 'normalize_late', 'normalize_early_leave', 'auto_close',
      'policy_recalculation', 'cancel_check_in', 'cancel_check_out', 'cancel_leave',
      'set_unauthorized_absence', 'cancel_unauthorized_absence'
    )
  );

create function public.attendance_admin_unauthorized_absence_v1(
  p_action text,
...
begin
  ...
  if p_action = 'set_unauthorized_absence' then
    ...
    insert into public.attendance_records (
      ...
      0, 0, btrim(p_reason), 'approved', p_actor_user_id::text,
      now(), false, now()
    ...
    insert into public.attendance_record_audit_logs (
      ...
      p_action, p_actor_user_id, null, to_jsonb(v_created), btrim(p_reason)
    );
  ...
  insert into public.attendance_record_audit_logs (
    ...
    p_action, p_actor_user_id, to_jsonb(v_record), null, btrim(p_reason)
  );
...
revoke all on function public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text) from public;
revoke all on function public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text) from anon;
revoke all on function public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text) from authenticated;
grant execute on function public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text) to service_role;

comment on function public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)
  is 'Atomically sets/cancels a confirmed unauthorized absence with policy, paid-payroll lock, and audit validation. Service-role API only.';

-- Postflight: schema only. This migration performs no attendance data backfill.
do $$
begin
  if to_regprocedure('public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)') is null then
    raise exception 'postflight failed: unauthorized absence RPC missing';
  end if;
  if has_function_privilege('public', 'public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)', 'EXECUTE')
    or has_function_privilege('anon', 'public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)', 'EXECUTE')
    or has_function_privilege('authenticated', 'public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)', 'EXECUTE')
    or not has_function_privilege('service_role', 'public.attendance_admin_unauthorized_absence_v1(text,bigint,date,bigint,text)', 'EXECUTE')
  then raise exception 'postflight failed: unexpected unauthorized absence RPC grants'; end if;
end
$$;
