...
create or replace function public.employee_update_profile_and_level_v2(
  p_user_id bigint,
  p_updates jsonb,
...
begin
  ...
  if v_before.is_system_account
    and v_before.level_base_date_override is distinct from p_base_date_override
    and p_base_date_override is not null then
    raise exception 'SYSTEM_ACCOUNT_NOT_ELIGIBLE' using errcode = '22023';
  end if;
  if v_before.termination_date is not null
    and v_before.level_base_date_override is distinct from p_base_date_override then
  ...
  if not v_before.is_system_account and v_hire_date is null then
  ...
  if p_base_date_override is not null and p_base_date_override < v_hire_date then
    raise exception 'BASE_DATE_BEFORE_HIRE_DATE' using errcode = '22023';
  end if;
  if p_base_date_override is not null
    and p_base_date_override > (now() at time zone 'Asia/Ho_Chi_Minh')::date then
    raise exception 'BASE_DATE_IN_FUTURE' using errcode = '22023';
  end if;
  if p_base_date_override is not null
    and v_termination_date is not null
    and p_base_date_override > v_termination_date then
  ...
  set
    ...
    role = case when p_updates ? 'role' then nullif(btrim(p_updates ->> 'role'), '') else v_before.role end,
    ...
    work_start_time = case when p_updates ? 'work_start_time' then nullif(p_updates ->> 'work_start_time', '')::time else v_before.work_start_time end,
    work_end_time = case when p_updates ? 'work_end_time' then nullif(p_updates ->> 'work_end_time', '')::time else v_before.work_end_time end,
  ...
  if not v_before.is_system_account
    and v_before.level_base_date_override is distinct from v_after.level_base_date_override then
    v_action := case
      when v_before.level_base_date_override is not null
        and v_after.level_base_date_override is null
        then 'level_base_date_reset_to_hire_date'
...
revoke all on function public.employee_update_profile_and_level_v2(bigint,jsonb,date,bigint,text)
  from public, anon, authenticated;
grant execute on function public.employee_update_profile_and_level_v2(bigint,jsonb,date,bigint,text)
  to service_role;

comment on function public.employee_update_profile_and_level_v2(bigint,jsonb,date,bigint,text) is
  'Atomically updates an employee profile and level base-date override. level_program_enabled remains untouched for legacy compatibility.';
