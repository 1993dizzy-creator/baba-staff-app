...
comment on function public.ledger_adjust_open_meal_transaction_v1(bigint,numeric,text,bigint) is
  'Adds a serialized delta correction for a confirmed attendance meal in an open recognition month; all confirmed linked ledger corrections are included in the effective amount while preserving original transaction and candidate provenance.';
