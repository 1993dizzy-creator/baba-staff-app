...
alter table public.employee_level_audit_logs
  drop constraint employee_level_audit_logs_previous_level_check,
  drop constraint employee_level_audit_logs_next_level_check;

update public.employee_level_audit_logs
set
  previous_level = case
    when previous_level between 1 and 8 then previous_level - 1
    else previous_level
  end,
  next_level = case
    when next_level between 1 and 8 then next_level - 1
    else next_level
  end
where previous_level between 1 and 8 or next_level between 1 and 8;

alter table public.employee_level_audit_logs
  add constraint employee_level_audit_logs_previous_level_check check (
    previous_level is null or previous_level between 0 and 7
  ),
  add constraint employee_level_audit_logs_next_level_check check (
    next_level is null or next_level between 0 and 7
  );
...
comment on column public.users.level_program_enabled is
  'Manual employee-level inclusion switch for owner/master roles. Other non-system employee roles are included by application policy.';
